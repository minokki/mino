...
<body>
   <div class="container" align="center">
   	...
   	<div class="form-group input-group">
   		...
   		<textarea class="form-control" name="board_content" id="board_content" cola="40" rows="15" readonly>${board.getBoard_content() }</textarea>  	
   	</div>
   	...
   	<c:if test="${board.getBoard_file() !=null}">
   		...
   		<input type="text" class="form-control" name="board_file" id="board_file" value="${board.getBoard_file() }" readonly/>  	
   		</div>
   	</c:if>
   	...
   	<div class="form-group input-group">
        ...
        <a href="boardModifyForm.bo?board_num=<%=board.getBoard_num()%>&p=<%=p%>&f=<%=f %>&q=<%=q %>" class="btn btn-success mr-sm-3">수정</a>
   		<a href="boardDeleteFrom.bo?board_num=<%=board.getBoard_num()%>&p=<%=p%>&f=<%=f %>&q=<%=q %>" class="btn btn-success mr-sm-3">삭제</a>
   		<a href="boardReplyFrom.bo?board_num=<%=board.getBoard_num()%>&p=<%=p%>&f=<%=f %>&q=<%=q %>" class="btn btn-success mr-sm-3">답변</a>
